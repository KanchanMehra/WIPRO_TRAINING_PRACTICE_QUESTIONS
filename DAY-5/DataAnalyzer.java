...
Description:
You are working on a data analysis module that needs to process different types of data. 
Implement a class named DataAnalyzer with the following requirements:

Constructor:
The class should have a constructor that takes an array of type T as its parameter and 
initializes an internal data structure to store the provided array.
...
Ensure that the method works for numeric types (e.g., Integer, Double) and does not require a separate implementation
for each type.

Find Maximum Method:
Implement a method named findMaximum that finds and returns the maximum element in the array.
Ensure that the method works for comparable types (e.g., Integer, String) and does not require a 
